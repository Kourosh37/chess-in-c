...
void gui_screen_lobby(struct ChessApp* app) {
    ...
    gui_draw_text("Host with a code or join with a code.",
             (int)panel.x + 34,
             (int)panel.y + 84,
             22,
    ...
    gui_draw_text("Host / Join", (int)left_card.x + 16, (int)left_card.y + 14, 30, palette->text_primary);
    ...
    gui_draw_text("Invite Code", (int)left_card.x + 16, (int)left_card.y + 136, 23, palette->text_primary);
    ...
    if (app->lobby_code[0] != '\0') {
        ...
        gui_draw_text("Your Host Code", (int)host_code_box.x + 12, (int)host_code_box.y + 10, 21, palette->text_secondary);
        gui_draw_text(app->lobby_code, (int)host_code_box.x + 14, (int)host_code_box.y + 36, 30, palette->accent);
    ...
    gui_draw_text("Status", (int)status_box.x + 12, (int)status_box.y + 10, 24, palette->text_primary);
    gui_draw_text(app->lobby_status, (int)status_box.x + 12, (int)status_box.y + 46, 22, palette->text_primary);
    ...
    gui_draw_text("Active Online Session", (int)right_card.x + 16, (int)right_card.y + 14, 30, palette->text_primary);
    ...
    if (app->online_match_active) {
        gui_draw_text("Match: Online Game", (int)session_box.x + 14, (int)session_box.y + 14, 24, palette->text_primary);

        if (app->online_match_code[0] != '\0') {
            char code_line[64];
            snprintf(code_line, sizeof(code_line), "Code: %s", app->online_match_code);
            gui_draw_text(code_line, (int)session_box.x + 14, (int)session_box.y + 50, 22, palette->text_secondary);
        }

        gui_draw_text(app->online_runtime_status, (int)session_box.x + 14, (int)session_box.y + 82, 21, palette->text_secondary);
        ...
        gui_draw_text("No active match in memory.", (int)session_box.x + 14, (int)session_box.y + 14, 24, palette->text_primary);
        gui_draw_text("When you leave a live online game to menu,", (int)session_box.x + 14, (int)session_box.y + 52, 20, palette->text_secondary);
        gui_draw_text("it appears here and you can resume/rejoin.", (int)session_box.x + 14, (int)session_box.y + 78, 20, palette->text_secondary);
    }
}
